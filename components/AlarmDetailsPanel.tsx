       ...
       <div className="flex-1 overflow-y-auto p-8 space-y-8 bg-slate-50/50">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
              <button 
                ...
                className={`w-full flex items-center justify-between px-4 py-3 transition-colors ${isUnitsOpen ? 'bg-slate-50 text-slate-900 border-b border-slate-100' : 'hover:bg-slate-50 text-slate-600'}`}
              >
                  <div className="flex items-center gap-3">
                    <div className={`p-1 rounded-lg ${isUnitsOpen ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-500'}`}>
                        <Navigation className="w-4 h-4" />
                    </div>
                    <span className="text-sm font-bold uppercase tracking-wider">Կարգախմբերի Գործողություններ</span>
                  </div>
                  <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${isUnitsOpen ? 'rotate-90' : ''}`} />
              </button>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
            <button 
              ...
              className={`w-full flex items-center justify-between px-4 py-3 transition-colors ${isHistoryOpen ? 'bg-slate-50 text-slate-900 border-b border-slate-100' : 'hover:bg-slate-50 text-slate-600'}`}
            >
                <div className="flex items-center gap-3">
                  <div className={`p-1 rounded-lg ${isHistoryOpen ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-500'}`}>
